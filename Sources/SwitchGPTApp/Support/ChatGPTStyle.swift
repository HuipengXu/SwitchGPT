...
struct ChatGPTDestructiveButtonStyle: ButtonStyle {
  @Environment(\.isEnabled) private var isEnabled
    ...
    configuration.label
      ...
      .background(
        configuration.isPressed
          ? ChatGPTStyle.semanticFill(ChatGPTStyle.dangerRed)
          : Color.clear,
        in: RoundedRectangle(cornerRadius: ChatGPTStyle.rowRadius, style: .continuous)
      )
      .overlay {
        RoundedRectangle(cornerRadius: ChatGPTStyle.rowRadius, style: .continuous)
          .stroke(ChatGPTStyle.semanticBorder(ChatGPTStyle.dangerRed), lineWidth: 1)
      }
      .opacity(isEnabled ? 1 : 0.42)
      .animation(.easeOut(duration: 0.15), value: configuration.isPressed)
  }
