...
public final class SwitchGPTAppStore {
  ...
  public func bootstrapCurrentAccountIfNeeded() async {
    guard !activity.isBusy, Self.isUntouchedDemoCatalog(accounts) else { return }
  ...
  public func removeAccount(_ accountID: AccountID) {
    ...
    if currentAccountID == removed.id, let replacement = accounts.first {
      currentAccountID = replacement.id
    }
    if persistState() {
  ...
  public func refreshIfStale(maxAge: TimeInterval, now: Date = Date()) async {
  ...
  public func simulateSwitch(to targetID: AccountID) async {
    guard let target = accounts.first(where: { $0.id == targetID }), target.id != currentAccountID
    else {
      return
    }
    guard !activity.isBusy else { return }
