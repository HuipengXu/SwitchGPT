...
public enum PreviewStatePersistenceError: Error, Equatable, LocalizedError, Sendable {
  ...
  public var errorDescription: String? {
    ...
    case .unsupportedAccountSource:
      return "Only mock accounts may be persisted by the preview app."
    case .invalidFile:
...
/// Stores only mock account metadata in the app's private Application Support directory.
/// It deliberately rejects codexHome sources and identity hashes so credentials cannot be
/// persisted through the preview UI by accident.
public struct PreviewStateFileStore: PreviewStatePersisting, Sendable {
  ...
  private func validate(_ state: PreviewState) throws {
    ...
    for account in state.accounts {
      ...
      guard case .mock = account.source, account.identityHash == nil else {
        throw PreviewStatePersistenceError.unsupportedAccountSource
      }
    }

    guard state.accounts.contains(where: { $0.id == state.currentAccountID }) else {
