...
当前结果为 26/26 场景通过，其中 17 个场景发生真实异常子进程退出；SwiftPM 单元测试为 160/160。新增独立 supervisor 测试拒绝 submitted job、嵌入式开发宿主、目标 app/祖先进程、错误签名和预期 bundle 外可执行文件，并证明 recovery 在首个停止动作前就绪。Desktop Integration 测试覆盖真实认证文件原子安装、私有档案、崩溃后新进程恢复、同队 helper、symlink/权限拒绝、仅对原始 PID 的单次 `SIGTERM` 后备和固定启动预算。AppCore 测试覆盖自动导入、app 托管登录、活动账号私有保存、身份 pin、重复身份清理、协议解码、任意账号数量、`700/600` 本地元数据持久化，以及不可覆盖的 metadata-only 终态回执。

## 尚未证明

- 新真实 ChatGPT Desktop 适配器的 A↔B→A 已现场通过；失败回滚终态回执与 Chat/Work 可见身份仍待最终验收。
- 嵌入式 one-shot supervisor 在真实事务崩溃后的现场验收；系统重启恢复仍未与真实事务目录接线。
...
- 运行中任务的可靠自动检测；当前 UI 每次要求人工明确确认。
- Keychain 凭据封装和临时认证物化。
- 真实客户端版本兼容矩阵与正式支持策略；未验证但经官方签名验证的版本只能在同一份明确确认下尝试一次，目标身份无法验证时沿既有 one-shot 回滚路径恢复原账号，绝不重试目标。

因此，`SwitchGPTSafetyCore` 本身仍是纯安全核心和夹具模拟器；可运行的 `SwitchGPTApp` 已接入真实额度、app 托管登录和默认关闭的真实切换适配器。离线接线只允许进入下一次受控闸门，不代表 Phase 0 或公开发布闸门已经通过。
