...
final class RuntimeControlSurfaceTests: XCTestCase {
  ...
  func testAccountRemovalIsVisibleAndConfirmed() throws {
    let card = try String(
      contentsOf: repositoryRoot.appendingPathComponent(
        "Sources/SwitchGPTApp/Views/AccountCardView.swift"
      ),
      encoding: .utf8
    )
    let dashboard = try String(
    ...
    XCTAssertTrue(card.contains("Remove account"))
    XCTAssertTrue(card.contains("ChatGPTDestructiveButtonStyle"))
    XCTAssertTrue(card.contains(".contentShape(Rectangle())"))
    XCTAssertTrue(dashboard.contains("confirmationDialog(\"Remove account?\""))
    XCTAssertTrue(dashboard.contains("This does not delete the OpenAI account."))
    XCTAssertTrue(app.contains("initialAccounts: []"))
  }
  ...
  func testDashboardKeepsItsCoreControlsOutOfSettings() throws {
    ...
    XCTAssertTrue(app.contains(".menuBarExtraStyle(.window)"))
    XCTAssertTrue(menu.contains(".frame(width: 390)"))
    ...
    XCTAssertFalse(menu.contains("refreshFeedback"))
    XCTAssertFalse(menu.contains("Task.sleep(for: .milliseconds(120))"))
    XCTAssertTrue(menu.contains("reportsActivity: false"))
    XCTAssertTrue(menu.contains("onHover"))
    XCTAssertTrue(menu.contains("Color.accentColor.opacity(0.12)"))
