...
artifact_root="$repository_root/.build/lifecycle-reboot"
...
if [[ "$artifact_root" != "$repository_root/.build/lifecycle-reboot" ]]; then
...
"$contents_path/MacOS/SwitchGPTLifecycleHost" phase-c-delivery-status
"$repository_root/Scripts/audit-lifecycle-reboot-capabilities.sh"
