...
const accounts = [
  { name: 'Current account', quota: 15, kind: 'person' },
  { name: 'Account 2', quota: 70, kind: 'briefcase' },
  { name: 'Account 3', quota: 42, kind: 'flask' },
  { name: 'Account 4', quota: 86, kind: 'building' },
]
  ...
  return (
    <section className="hero section-frame" id="top">
      <div className="hero-copy">
        ...
        <p>A public macOS alpha for viewing multiple local accounts with explicit read-only quota access.</p>
  ...
  return (
    <section className="product-section section-frame" id="product">
      ...
      <div className="product-section-copy">
        ...
        <p>Sign in with OpenAI and SwitchGPT manages each account privately. No folders, tokens, or developer settings to configure.</p>
        <div className="feature-list">
          <FeatureRow icon="accounts" title="Simple account sign-in" copy="Add any number of accounts through the official sign-in flow." />
          ...
          <FeatureRow icon="safety" title="Explicit switching" copy="Real desktop switching is experimental, default-off, and requires confirmation every time." />
        </div>
      </div>
    </section>
  )
      ...
      <div className="safety-inner section-frame">
        <div className="safety-copy">
          ...
          <div className="safety-notes">
            <p><span className="note-icon">◇</span>Real switching is experimental and explicit.</p>
            <p><span className="note-icon">&lt;/&gt;</span>Credentials stay in private local storage.</p>
          </div>
        </div>
        <div className="boundary-flow" aria-label="SwitchGPT safety boundary">
          <BoundaryStep title="Stage a read-only copy" kind="data" />
          <ArrowIcon />
          <BoundaryStep title="Display quota and status" kind="display" />
          <ArrowIcon />
          <BoundaryStep title="Explicit only" copy="Real switching requires confirmation" kind="switch" />
        </div>
      </div>
    </section>
  )
  ...
  return (
    <section className="process-section section-frame" id="process">
      ...
      <div className="process-list">
        <ProcessStep number="1" title="Sign in" copy="Add another account through the official OpenAI sign-in flow." />
        <ArrowIcon />
        <ProcessStep number="2" title="Review" copy="See weekly quota and optional five-hour usage at a glance." />
        <ArrowIcon />
        <ProcessStep number="3" title="Confirm" copy="Switch only after the safety preflight passes and you confirm the restart." />
      </div>
    </section>
  )
}
